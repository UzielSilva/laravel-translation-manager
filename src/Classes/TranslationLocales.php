...
class TranslationLocales
{
    ...
    private static function natSort($arr)
    {
        natsort($arr);
        return $arr;
    }
    ...
    public function normalize()
    {
        ...
        if (!is_array($addConfigLocales)) {
            $addConfigLocales = array($addConfigLocales);
        }
        ...
        if (array_search($currentLocale, $locales) === false) {
            $currentLocale = $appLocale;
        }
        if (array_search($primaryLocale, $locales) === false) {
        ...
        if (!$translatingLocale) {
            $userTranslatableLocales = self::natSort(array_diff($userLocales, array($primaryLocale)));
        ...
        if (!$displayLocales || count($displayLocales) === 0) {
            $displayLocales = [$primaryLocale, $translatingLocale];
        }
        ...
        if ($translatingLocale !== $primaryLocale) {
            $firstLocales[] = $translatingLocale;
        }
        $displayLocales = self::natSort(array_diff($displayLocales, $firstLocales));
        ...
        $locales = array_values(array_merge($displayLocales, self::natSort(array_diff($locales, $displayLocales))));
