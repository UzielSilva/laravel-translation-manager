...
SELECT source FROM $ltm_translations WHERE "group" = ? AND "key" LIKE BINARY ?
    ...
    {
        $ltm_translations = $this->getTranslationsTableName();
        $sql = $this->adjustTranslationTable(
            "INSERT INTO $ltm_translations (status, locale, \"group\", \"key\", \"value\", created_at, updated_at, source, saved_value, is_deleted, was_used) VALUES "
                . implode(",", $values)
        );
